...
export function initAdmin(page,updates={}){
    ...
    document.addEventListener("click",async event=>{
        ...
        const context={objects,parentId:object?.id,recordTypes:availableRecordTypes,subjectTypes,types,photos,sources,records,children,subjects,updates};
        if(action==="edit-object"){
            setModalUrl("editor",{entityId:object.id,entityType:"object"});
        ...
        if(action==="add-object"){
            setModalUrl("editor",{entityId:null,entityType:"object"});
            await openEditor("object",null,{...context,children:[]});
            return;
        }
        if(action==="add-photo"){
            setModalUrl("editor",{entityId:null,entityType:"photo"});
            await openEditor("photo",null,context);
            return;
        }
        if(action==="edit-photo"){
            const photo=photos.find(photo=>photo.id===id);
            if(!photo)return;
            setModalUrl("editor",{entityId:photo.id,entityType:"photo"});
            await openEditor("photo",photo,{...context,photos});
            return;
        }
        ...
        if(action==="add-source"){
            setModalUrl("editor",{entityId:null,entityType:"source"});
            await openEditor("source",null,context);
            return;
        }
        if(action==="edit-source"){
            const source=sources.find(source=>source.id===id);
            if(!source)return;
            setModalUrl("editor",{entityId:source.id,entityType:"source"});
            await openEditor("source",source,{...context,sources});
            return;
        }
        ...
        if(action==="add-record"){
            setModalUrl("editor",{entityId:null,entityType:"record"});
            await openEditor("record",null,context);
            return;
        }
        if(action==="edit-record"){
            const record=records.find(record=>record.id===id);
            if(!record)return;
            setModalUrl("editor",{entityId:record.id,entityType:"record"});
            await openEditor("record",record,{...context,records});
            return;
        }
        ...
        if(action==="add-subject"){
            setModalUrl("editor",{entityId:null,entityType:"subject"});
            await openEditor("subject",null,{...context,updates:{...updates,updateSubjectBlock:refreshSubjectsModal,onSubjectDeleted:refreshSubjectsModal}});
            return;
        }
        if(action==="edit-subject"){
            const subject=subjects.find(subject=>subject.id===id);
            if(!subject)return;
            setModalUrl("editor",{entityId:subject.id,entityType:"subject"});
            await openEditor("subject",subject,{...context,subjects,updates:{...updates,updateSubjectBlock:refreshSubjectsModal,onSubjectDeleted:refreshSubjectsModal}});
            return;
        }
        if(action==="delete-subject"){
            if(!confirm("Удалить субъект?"))return;
            try{
                await deleteEntity("subject",id,{...context,subjects,updates:{...updates,onSubjectDeleted:refreshSubjectsModal}});
            }catch(error){
