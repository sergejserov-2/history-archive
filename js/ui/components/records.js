...
function compareItems(a,b){
    ...
    const metaCompare=aMeta.localeCompare(bMeta,"ru",{
        numeric:true,
        sensitivity:"base"
    });
...
export function renderRecord(record,subjects=[]){
    ...
    return`
        <div class="${rowClass}" data-id="${record.id}">
...
function getInsertResult(record,subjects=[],recordTypes=[]){
    ...
    return insertEntityListItem({
        ...
        compare:(newElement,row)=>{
            const meta=
                row.querySelector(".entity-list-row__meta")
                    ?.textContent.trim()??"";

            const title=
                row.querySelector(".entity-list-row__title-text")
                    ?.textContent.trim()??"";
...
export function renderRecords(records,recordTypes=[],subjects=[]){
    ...
    typedRecords.forEach(group=>groups.push({
        ...
        items:group.records.map(
            record=>getRecordData(record,subjects)
        )
    }));
