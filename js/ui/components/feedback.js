...
export function openFeedbackModal(feedback,{fromUrl=false}={}){
    ...
    if(!fromUrl)setModalUrl("feedback-view",{entityId:feedback.id});
...
function renderFeedback(feedback){
    ...
    return`<div class="feedback">
        ...
        <div class="feedback__author"><div class="feedback__name">${name}</div><div class="feedback__date">${date}</div></div>
        ${photoIds.length?renderFeedbackPhotos(photoIds):""}
        ${email?`<div class="feedback__contact">Для обратной связи: <a href="mailto:${escapeAttribute(email)}">${escapeHTML(email)}</a></div>`:""}
    </div>`;
}
// ==========================================
// Photos
...
function setupFeedbackPhotos(root,feedback){
    ...
    const gallery=photoIds.map((photo,index)=>({id:`${feedback.id}-${index}`,title:"",description:"",previewPath:photo.previewPath,storagePath:photo.storagePath}));
