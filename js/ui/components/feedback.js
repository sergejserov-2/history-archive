    ...
    return`
        <div class="feedback__photos">
            ${photoIds.map((photo,index)=>`
                <div class="feedback__photo" data-photo-index="${index}">
                    <img src="${escapeAttribute(photo.previewPath||photo.storagePath||"")}" alt="" loading="lazy">
                </div>
            `).join("")}
        </div>
    `;
...
function setupFeedbackPhotos(root,feedback){
    ...
    root.querySelector(".feedback__photos")?.addEventListener("click",event=>{
        ...
        openPhotoViewer(gallery[index],{
            photos:gallery,
            showInfo:false,
            urlParams:{feedbackId:feedback.id}
        });
    });
}
