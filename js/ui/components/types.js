...
export async function openTypesModal(){
    ...
    modal.root.addEventListener("click",async event=>{
        ...
        if(
            ...
            const type=action.replace(
                "edit-",
                ""
            );

            const typeEntity=getTypeById(
                type,
                id,
                modal
            );
        ...
        if(
            ...
            if(
                !confirm(
                    `Удалить тип «${typeEntity.title??id}»?`
                )
            ){
                return;
            }
...
export async function refreshTypesModal(){
    ...
    currentTypesModal.objectTypes=
        objectTypes;
    currentTypesModal.recordTypes=
        recordTypes;
    currentTypesModal.subjectTypes=
        subjectTypes;
...
function renderTypesList(
    ...
    const objectItems=objectTypes.map(
        type=>
            createTypeItem(
                type,
                "objectType",
    ...
    const recordItems=recordTypes.map(
        type=>
            createTypeItem(
                type,
                "recordType",
    ...
    const subjectItems=subjectTypes.map(
        type=>
            createTypeItem(
                type,
                "subjectType",
