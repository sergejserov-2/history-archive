...
export async function renderChildren(children,ADMIN_MODE=false,currentObject=null,objects=[],types=[]){
    ...
    const currentType=typeMap.get(currentObject?.typeId);
    ...
    if(Array.isArray(currentType?.levels)){
        ...
        levels=[currentType.level];
    }

    const canHaveChildren=!levels.includes(0);
    ...
    const preparedChildren=await Promise.all((children??[]).map(async child=>{
        ...
        const parentRelations=Array.isArray(child.parents)
            ?child.parents
            :[];

        const addressLines=parentRelations
            .map(parent=>{
            ...
            ?addressLines
                .map(line=>`
                    <div class="child-card__address-line">
                        ${line.parentAddress?`${line.parentAddress}, `:""}${line.childAddress}
                    </div>
                `)
                .join("")
            :"";
        ...
        return{
            ...
            sortAddress:addressLines
                .map(line=>`${line.parentAddress}, ${line.childAddress}`)
                .join(" ")
        };
    }));

    preparedChildren.sort((a,b)=>
