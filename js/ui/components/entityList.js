...
function renderGroup(group){
    const direction=group.sortDirection==="desc"?"desc":"asc";
    const items=[...(group.items??[])].sort((a,b)=>sortItems(a,b,direction));
    return`
        <div class="entity-list__group" data-entity-group="${group.id??group.title??""}">
            <div class="entity-list__group-title">${group.title??""}</div>
...
function sortItems(a,b,direction="asc"){
    if(a.sortValue!==undefined||b.sortValue!==undefined){
        ...
        if(Number.isFinite(aValue)&&Number.isFinite(bValue)&&aValue!==bValue)return direction==="desc"?bValue-aValue:aValue-bValue;
    }
    ...
    if(aNumber!==null&&bNumber===null)return direction==="desc"?1:-1;
    if(aNumber===null&&bNumber!==null)return direction==="desc"?-1:1;
    if(aNumber!==null&&bNumber!==null&&aNumber!==bNumber)return direction==="desc"?bNumber-aNumber:aNumber-bNumber;
    const metaCompare=aMeta.localeCompare(bMeta,"ru",{numeric:true,sensitivity:"base"});
    if(metaCompare!==0)return direction==="desc"?-metaCompare:metaCompare;
    const titleCompare=String(a.title??"").localeCompare(String(b.title??""),"ru",{sensitivity:"base"});
    return direction==="desc"?-titleCompare:titleCompare;
}
...
function renderRow(item){
    ...
    const rowClass=[
        "entity-list-row",
        item.clickable||item.href?"entity-list-row--clickable":"",
...
function getGroup(list,groupId){
    return[...list.querySelectorAll(".entity-list__group")].find(group=>group.dataset.entityGroup===String(groupId))||null;
}

export function insertEntityListItem({groupId,groupTitle,element,compare}={}){
    const list=document.querySelector(".entity-list");
    if(!list||!element)return null;
    let group=getGroup(list,groupId);
    const created=!group;
    if(!group){
        group=document.createElement("div");
        group.className="entity-list__group";
        group.dataset.entityGroup=groupId??groupTitle??"";
        group.innerHTML=`<div class="entity-list__group-title">${groupTitle??""}</div>`;
        list.appendChild(group);
    }
    const rows=[...group.querySelectorAll(".entity-list-row")];
    const before=typeof compare==="function"?rows.find(row=>compare(element,row)>0):null;
    if(before)group.insertBefore(element,before);
    else group.appendChild(element);
    return{element,group,created};
}
