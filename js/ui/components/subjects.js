...
function renderSubjectsList(subjects=[],subjectTypes=[]){
    ...
    const untypedItems=subjects
        .filter(subject=>!subject.typeId||!subjectTypes.some(type=>type.id===subject.typeId))
        .sort((a,b)=>(a.title??"").localeCompare(b.title??"","ru"))
        .map(subject=>({
            id:subject.id,
            clickable:true,
    ...
    return renderEntityList({
        groups,
        addButton:adminAdd("add-subject","Добавить субъект")
    });
}

function formatSubjectYears(subject){
    if(subject.dateStart&&subject.dateEnd)return`${escapeHTML(subject.dateStart)} – ${escapeHTML(subject.dateEnd)}`;
    if(subject.dateStart)return`с ${escapeHTML(subject.dateStart)}`;
    if(subject.dateEnd)return`до ${escapeHTML(subject.dateEnd)}`;
    return"";
}
