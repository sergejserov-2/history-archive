export function formatBoundary(value,prefix){
    if(!value)return"";
    let result=value;
...
export function getPeriod(entity){
    if(entity.dateMode==="date"){
        return entity.date||"—";
    }
    if(entity.dateStart&&entity.dateEnd){
        return`${entity.dateStart} – ${entity.dateEnd}`;
    }
    if(entity.dateStart){
        return formatBoundary(entity.dateStart,"с");
    }
    if(entity.dateEnd){
        return formatBoundary(entity.dateEnd,"до");
    }
    return"—";
}
