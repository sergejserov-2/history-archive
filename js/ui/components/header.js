...
if(favicon){
    ...
    const update=()=>{favicon.href=dark.matches?"icons/logoDark.svg":"icons/logoLight.svg";};
    update();
...
export function renderHeader(page=null,updates=null){
    const html=`
<header class="header">
    <a class="header__logo" id="headerLogo" href="index.html">
        <img src="icons/logoDark.svg" alt="Historical Archive">
        <span>Краснохолмское краеведение</span>
    </a>
...
`;
    ...
    setTimeout(async()=>{
        ...
        if(headerLogo){
            try{
                const object=await getHighestLevelObject(page?.objects,page?.types);
                if(object?.id)headerLogo.href=`object.html?id=${object.id}`;
            }catch(error){
                console.error("Не удалось определить главный объект:",error);
            }
        }
        ...
        const updateButtons=(ADMIN_MODE)=>{
            if(typesButton)typesButton.hidden=!ADMIN_MODE;
            loginButton.innerHTML=ADMIN_MODE?'<img src="icons/logout.svg" class="header-icon"> Выйти':'<img src="icons/login.svg" class="header-icon"> Войти';
            loginButton.classList.toggle("header__button--admin",ADMIN_MODE);
        };
